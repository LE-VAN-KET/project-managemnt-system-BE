...
public class RemoveUserRequestKafkaListener implements KafkaConsumer<MemberRequestModel> {
    private final MemberRequestMessageListener memberRequestMessageListener;

    @Override
    @KafkaListener(id = "${kafka-consumer-config.remove-user-consumer-group-id}",
            topics = "${member-service.remove-user-request-topic-name}")
    public void receive(@Payload List<MemberRequestModel> messages,
        ...
        messages.forEach(message -> {
            memberRequestMessageListener.removeUser(message.getUserId());
        });
    }
}
